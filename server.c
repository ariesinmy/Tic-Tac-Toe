...
void *gamemenu(void *arg)
{
	...
	while(1){
		...
		// Accept playername
			...
			pthread_mutex_lock(&in_game_mutex);
			playing[clientSocket] = player_sock; // clientSocket is playing with player_sock
			playing[player_sock] = clientSocket; // player_sock is playing with clientSocket
			pthread_mutex_unlock(&in_game_mutex);
			usleep(1000);
		...
		// match playername
			...
			//get player name(the invited one)
			player = strtok(buffer, " ");
			player = strtok(NULL, " ");
			player_sock = -1; //set player sock to -1
			...
			// if player socket < 0, two conditions: havent this player / he doesnt online
			...
			// if player socket > 0, player is exist and he is online
			// then send data to player_sock
			else{
				pthread_mutex_lock(&in_game_mutex);
				if(playing[player_sock] < 0){ // if this player is not playing game
				...
				else{ // if this player is playing game with other player
				      // then send response(Busy) to client
					memset(response, 0, sizeof(response));
