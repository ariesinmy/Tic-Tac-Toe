...
char playername[100][16]={"stdin","stdout","stderr","socket", "Player1",
			  "Player2","Player3","Player4","Player5","Player6"
			  "Player7","Player8","Player9","Player10","Player11"}; // actually fifteen fd connect in
void *gamemenu();
int client_number;
short online[100];
short playing[100];
...
static void sig_handler(int sig)
{
	...
	if(sig==SIGINT){
		for(int i=0; i<15; i++){
			if(online[i]==1) write(i, "quit", sizeof("quit")); // i is a file descriptor(number)
		}
		usleep(1000);
		close(serverSocket);
		exit(0);
...
void *gamemenu(void *arg)
{
	...
	while(1){
		...
		// If a client press 'Y', it will send 'Accept' to server for accpet the invition from other player
		//
			...
			if(player_sock<0){
				...
				pthread_mutex_lock(&in_game_mutex);
				if(playing[player_sock]<0){ // player_sock is not playing
					memset(data, 0, sizeof(data));
					printf("Matching Successful !!\n");
					strcpy(data, "Invite: ");
					strcat(data, playername[clientSocket]);
					strcat(data, " invites you (Y/N) ? ");
					write(player_sock, data, sizeof(data)); // send the invition to player_sock
					usleep(1000);
				}
				else{ // player_sock is on other game, send 'Busy' to client
					memset(response, 0, sizeof(response));
		...
		// In game, always send command 'Next' to server
		else if(strncmp(buffer, "Next", 4)==0){
			int location = -1;
			// get minsocket and maxsocket, and use corresponding gamemap struct
			int minsocket = playing[clientSocket] > clientSocket ? clientSocket : playing[clientSocket];
                        ...
                        char tempmark =  mark[clientSocket]==0 ? 'O' : 'X'; // If mark[clientSocket] equals zero, tempmark is 'O'
									    // Else tempmark is 'X'
			int winflag = 0;
			char temp1;
			temp = strtok(buffer, ";");
			temp = strtok(NULL, ";");
			// get the number location for testing winmap( |= with the player)
			location = 1 << (temp[0] - 48);
			printf("*******temp: %s \t location: %d *******\n", temp, location);
			...
			++gamemap[minsocket][maxsocket].count; // increase the turn of the game, if count == 9 , win or drew

			if(minsocket == clientSocket){
				gamemap[minsocket][maxsocket].player1 |= location;
				printf("gamemap[minsocket][maxsocket].player1 == %d\n", gamemap[minsocket][maxsocket].player1);
				for(int i=0; i<8; i++){
				...
				printf("gamemap[minsocket][maxsocket].player2 == %d\n", gamemap[minsocket][maxsocket].player2);
				for(int i=0; i<8; i++){
			...
			pthread_mutex_lock(&in_game_mutex);
			strcat(response, playername[playing[clientSocket]]); // return client player name to sock
			strcat(response, ";");
			...
			// save the newest gamemap to sock
			for(int i=0; i<3; i++){
                                strcat(response, gamemap[minsocket][maxsocket].map[i]);
          		}
                        strcat(response, ";");
			
			write(playing[clientSocket], response, sizeof(response)); // send response to client
			pthread_mutex_unlock(&in_game_mutex);
			write(clientSocket, response, sizeof(response)); // send response to player
		}
