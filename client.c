...
int main()
{
	...
	unsigned short serverPort = SERVERPORT; //8002
	struct sigaction sa;
...
void *recvsock(void *arg)
{
	...
	while(1){
		...
		n = read(sock, data, sizeof(data)); // Read sock to data(analyze data)

		if(strcmp(data, "quit")==0) break; // if data is quit, end this client
		
		else if(strncmp(data, "Invite", 6)==0){
			write(fileno(stdout), "\n", sizeof("\n")); // printf '\n' first
			write(fileno(stdout), data, sizeof(data)); //printf "Invite: xx invites you(Y/N) ?"
			temp = strtok(data, " "); // "Invite:"
			temp = strtok(NULL, " "); // "xx"(username)
			pthread_mutex_lock(&match_mutex); 
			strcpy(playername, temp); 
			match_flag=1; // set up match_flag as matching
...
void *sendsock(void *arg)
{
	...
	printf("\n**************** Welcome to the OOXX Game! ****************\n");
	printf("\t<< Game Menu >>\n");
	printf("(menu) Show the commands you can use\n");
	printf("(list) List the players who are online\n");
	...
	while(1){
		...
		if(strcmp(command, "quit")==0){
			...
			scanf("%s", playername);
			strcat(command, " ");
			strcat(command, playername); //command:[match playername] write in sock
			write(sock, command, sizeof(command));
			printf("\t\n Wait !!\n");
			pthread_mutex_lock(&data_mutex);
			pthread_mutex_unlock(&data_mutex);
			...
			// when enterflag is true, then enter game 'playing function'
			if(enterflag){
				playing(sock);
			}

			//reset match_flag to 0
			match_flag = 0;
		}
...
void playing(int socket)
{
	...
	while(1){
		...
		if(strcmp(data, "quit")==0 || strcmp(data, "Win")==0 || strcmp(data, "Lose")==0 || strncmp(data, "Even", 4)==0 || strncmp(data, "Leave", 5)==0){
			...
			memset(response, 0, sizeof(response));
			temp[1] = '\0';
			temp[0] = next + 48; // change to DataType 'char' and save into response
